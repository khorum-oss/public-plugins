...
class UploadToDigitalOceanSpacesServiceFunctionalTest : UnitTest() {
    ...
    val givenArtifactPath = "org/khorum/oss/my-lib/1.0.0"
    ...
    fun `uploadSpaces plugin library happy path`(@TempDir tempDir: Path) {
            ...
            s3Client.putObject(
                match<PutObjectRequest> {
                    it.bucket() == testBucket
                        && it.key() == "plugins/org/khorum/oss/my-lib/1.0.0/org.khorum.oss.my-lib.gradle.plugin-1.0.0.pom"
                        && it.acl() == ObjectCannedACL.PUBLIC_READ
                },
                match<Path> {
                    it.endsWith("libs/org.khorum.oss.my-lib.gradle.plugin-1.0.0.pom")
                }
            )
        ...
        every {
            s3Client.putObject(
                match<PutObjectRequest> {
                    it.bucket() == testBucket
                        && it.key() == "plugins/org/khorum/oss/my-lib/1.0.0/my-lib-1.0.0.jar"
                        && it.acl() == ObjectCannedACL.PUBLIC_READ
                },
                match<Path> {
        ...
        override fun pluginAdapters(): List<ProjectAdapter.MavenPublicationAdapter> {
            return listOf(
                object : ProjectAdapter.MavenPublicationAdapter {
                    override val groupId: String = "org.khorum.oss"
